...
function computeAngles(lms: Landmark[]): Record<string, number> {
  return tf.tidy(() => {
    ...
    const BA = tf.sub(As, Bs),
      BC = tf.sub(Cs, Bs);
    const dot = tf.sum(tf.mul(BA, BC), 1);
    const mag = tf.mul(
      tf.sqrt(tf.sum(tf.square(BA), 1)),
      tf.sqrt(tf.sum(tf.square(BC), 1)),
    );
...
function checkForm(lms: Landmark[], disc: string, subDiscipline?: string) {
  ...
  const rule = getPoseRules(targetKey) as Rule;
  if (!rule)
    return {
      isCorrect: true,
      score: 100,
  ...
  const avgVis =
    keyIdx.reduce((s, i) => s + (lms[i]?.visibility ?? 0), 0) / keyIdx.length;
  if (avgVis < (rule.visibilityMin ?? 0.45))
    return {
      isCorrect: true,
      score: 100,
      feedback: [],
  ...
  const results = rule.checks.map((c) => {
    ...
    return {
      joint: {
        ...
        expected: [c.min, c.max] as [number, number],
        ok,
      },
      cue: ok ? null : c.cue,
    };
  ...
  const score = Math.round(
    100 - (feedback.length / Math.max(rule.checks.length, 1)) * 100,
  );
  return {
    isCorrect: score >= 70,
    score,
...
self.addEventListener("message", async (e: MessageEvent) => {
  ...
  if (msg["type"] === "INIT") {
    ...
    const result = checkForm(
      (msg["landmarks"] as Landmark[]) ?? [],
      (msg["discipline"] as string) ?? "fitness",
      msg["subDiscipline"] as string | undefined,
    );
