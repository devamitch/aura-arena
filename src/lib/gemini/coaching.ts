...
export async function getGeminiCoaching(session: SessionData): Promise<CoachingResponse | null> {
...
export async function* streamGeminiCoaching(session: SessionData): AsyncGenerator<string> {
  const apiKey = (import.meta.env as Record<string, string>)['VITE_GEMINI_API_KEY'];
  if (!apiKey) return;
  try {
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:streamGenerateContent?key=${apiKey}&alt=sse`,
      {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          contents: [{ parts: [{ text: `You are ARIA, the AURA ARENA AI coach. Give a brief 2-sentence coaching tip for a ${session.discipline} athlete who scored ${session.finalScore}/100. Be encouraging and specific.` }] }],
    ...
    while (true) {
      ...
      for (const line of lines) {
        try {
          const json = JSON.parse(line.slice(6)) as { candidates?: { content?: { parts?: { text?: string }[] } }[] };
