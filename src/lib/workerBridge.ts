...
export class WorkerBridge {
  ...
  constructor(urlOrWorker: URL | Worker, opts: WorkerBridgeOptions = {}) {
    this._worker =
      urlOrWorker instanceof Worker
        ? urlOrWorker
        : new Worker(urlOrWorker, { type: "module" });
    if (opts.onMessage)
      this._worker.onmessage = (e) => {
        if (this._alive) opts.onMessage!(e);
      };
    if (opts.onError)
      this._worker.onerror = (e) => {
        if (this._alive) opts.onError!(e);
      };
  }
...
export function createWorkerBridge(
  urlOrWorker: URL | Worker,
  onMessage: MsgHandler,
  onError?: (msg: string) => void,
): WorkerBridge {
  return new WorkerBridge(urlOrWorker, {
    onMessage,
