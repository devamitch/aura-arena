...
export const renderFrameToCanvas = (
  ...
  if (drawScanLine && scanProgress < 1)
    renderScanLine(ctx, W, H, color, scanProgress);
};
...
const renderSkeleton = (
  ...
  for (const lms of all) {
    ...
    for (const [a, b] of POSE_CONN) {
      ...
      // We removed the visibility < 0.3 threshold here so that partial detections still draw
      ctx.beginPath();
