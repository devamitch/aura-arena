...
import type { RhythmState } from '@lib/scoreEngine';
import type { FrameScore, Landmark } from '@types';
import { useCallback, useEffect, useRef, useState } from 'react';
import type { WorkerFrameResult } from '../workers/pose.worker';
  ...
  useEffect(() => {
    ...
    worker.onmessage = (e: MessageEvent) => {
      ...
      if (msg.type === 'READY') {
        ...
        busyRef.current = false;
        setIsBusy(false);
        setWorkerError((msg.message as string | undefined) ?? 'Unknown worker error');
      }
    };
