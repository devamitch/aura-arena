...
export function useScoringWorker(
    ...
    frameRef.current = new WorkerBridge(
      new URL("../workers/scoring/frame.worker.ts", import.meta.url),
      {
        onMessage: (e) => {
          ...
          if (m["type"] === "READY")
            setState((s) => ({ ...s, frameReady: true }));
          else if (m["type"] === "FRAME_SCORED") {
            lastScore.current = m["frameScore"] as FrameScore;
            bus.emit(
              "frame:scored",
              m as Parameters<typeof bus.emit<"frame:scored">>[1],
            );
          }
        },
        onError: () => {},
    ...
    correctRef.current = new WorkerBridge(
      new URL("../workers/scoring/correctness.worker.ts", import.meta.url),
      {
        onMessage: (e) => {
          const m = e.data as Record<string, unknown>;
          if (m["type"] === "READY")
            setState((s) => ({ ...s, correctnessReady: true }));
          else if (m["type"] === "FORM_RESULT") {
            lastCorrectness.current = m["poseCorrectness"] as PoseCorrectness;
            bus.emit("form:result", { poseCorrectness: m["poseCorrectness"] });
          }
        },
        onError: () => {},
      },
    ...
    sessionRef.current = new WorkerBridge(
      new URL("../workers/scoring/session.worker.ts", import.meta.url),
      {
        onMessage: (e) => {
          const m = e.data as Record<string, unknown>;
          if (m["type"] === "READY")
            setState((s) => ({ ...s, sessionReady: true }));
          else if (m["type"] === "SUMMARY") {
            summaryResolve.current?.(m["summary"]);
            summaryResolve.current = null;
            bus.emit("session:summary", { summary: m["summary"] });
          }
        },
        onError: () => {},
      },
  ...
  const scoreFrame = useCallback(
    (payload: Record<string, unknown>) =>
      frameRef.current?.post({ type: "SCORE_FRAME", ...payload }),
    [],
  );
  const checkForm = useCallback(
    (lms: unknown[], disc: string, sub?: string) =>
      correctRef.current?.post({
        type: "CHECK_FORM",
        landmarks: lms,
        discipline: disc,
        subDiscipline: sub,
  ...
  const addFrame = useCallback(
    (fs: FrameScore) =>
      sessionRef.current?.post({ type: "ADD_FRAME", frameScore: fs }),
    [],
  );
  const resetSession = useCallback(
    () => sessionRef.current?.post({ type: "RESET" }),
    [],
  );
