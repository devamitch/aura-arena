...
export default function TrainingPage() {
  ...
  const [geminiCoaching, setGeminiCoaching] = useState<GeminiCoaching | null>(
    null,
  );
  ...
  const handleToggleRecording = useCallback(() => {
    if (recorder.isRecording) {
      recorder.stopRecording().then((result) => setRecordedUrl(result.url)).catch(() => {});
      return;
    }
    setRecordedUrl(null);
  ...
  const handleEndSession = useCallback(() => {
    ...
    if (recorder.isRecording) {
      recorder
        .stopRecording()
        .then((res) => setRecordedUrl(res.url))
        .catch(() => {});
    }
    camera.stopCamera();
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black overflow-hidden font-sans">
      ...
      <AnimatePresence>
        {camera.engineReady &&
          camera.poseCorrectness.exercise !== "idle" &&
          !camera.outOfFrame && (
            <motion.div
            ...
            >
              ...
              <div
                ...
                style={{
                  background: camera.poseCorrectness.isCorrect
                    ? "rgba(34,197,94,0.18)"
                    : "rgba(239,68,68,0.18)",
              ...
              >
                ...
                <div>
                  <p
                    ...
                    style={{
                      color: camera.poseCorrectness.isCorrect
                        ? "#4ade80"
                        : "#f87171",
                    }}
                  >
              ...
              {/* Feedback cues */}
              {!camera.poseCorrectness.isCorrect &&
                camera.poseCorrectness.feedback.slice(0, 2).map((cue, i) => (
                  <motion.div
                    key={i}
      ...
      <div className="absolute top-24 right-5 z-20 flex flex-col items-end gap-2">
        ...
        <AnimatePresence>
          {recordedUrl && !recorder.isRecording && (
            <a
              href={recordedUrl}
              download={`AURA_ARENA_${Date.now()}.webm`}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl backdrop-blur-xl"
              style={{
            ...
            <div className="w-full max-w-sm px-6 pb-safe-bottom pb-8 flex flex-col gap-4">
              ...
              <div className="flex flex-col items-center gap-1 mb-2">
                <div
                  ...
                  style={{
                    background: "rgba(var(--ac-rgb,0,240,255),0.12)",
                    border: "2px solid rgba(var(--ac-rgb,0,240,255),0.5)",
                    boxShadow: "0 0 40px rgba(var(--ac-rgb,0,240,255),0.2)",
                  }}
                >
                  <span
                    ...
                    style={{ color: "var(--ac)" }}
                  >
                    {getGrade(sessionResult.finalScore)}
                  </span>
                </div>
                <p className="text-white font-black text-2xl tabular-nums">
                  {sessionResult.finalScore}
              ...
              <div className="grid grid-cols-3 gap-2">
                {(
                  [
                    ...
                    ["Timing", sessionResult.timing],
                    ["Power", sessionResult.power],
                  ] as const
                ).map(([label, val]) => (
                  <div
                    key={label}
                ...
                {geminiCoaching ? (
                  ...
                  !geminiLoading && (
                    <p className="text-white/40 text-xs">
                      Add VITE_GEMINI_API_KEY for AI coaching
                    </p>
                  )
                )}
              </div>
              ...
              <motion.button
                ...
                style={{
                  background:
                    "linear-gradient(135deg, var(--ac) 0%, #0088cc 100%)",
                  color: "#040914",
                }}
              >
                Continue
      ...
      <AnimatePresence>
        {camera.errorMessage && (
          ...
          >
            ...
            <p className="text-white/30 text-xs text-center px-10 mb-6">
              {camera.errorMessage}
            </p>
            <button
