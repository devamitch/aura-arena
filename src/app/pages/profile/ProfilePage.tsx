...
import { Star, BarChart2, Brain, Trophy, Settings, ChevronRight, Edit2, LogOut, Volume2, Eye } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts';
import { useUser, useXP, useTier, useDailyStreak, useSessionHistory, useEarnedAchievements, useStore, useSoundEnabled, useReduceMotion, useMasterVolume } from '@store';
...
import { ACHIEVEMENTS, TIERS } from '@utils/constants';
...
const RARITY_C: Record<string,string> = {Common:'#6b7280',Rare:'#60a5fa',Epic:'#c084fc',Legendary:'#fbbf24'};
...
export default function ProfilePage() {
  ...
  const user      = useUser();
  const xp        = useXP();
  const { discipline:disc, subDiscipline, accentColor, currentTier, nextTier, tierProgress, avgScore } = usePersonalization();
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" style={{background:'var(--void)'}}>
      ...
      <div className="px-5 pt-6 pb-3 flex-shrink-0 relative overflow-hidden">
        <div className="absolute -top-10 -right-10 w-52 h-52 rounded-full opacity-10 blur-3xl pointer-events-none"
          style={{background:accentColor}}/>
        <div className="flex items-center justify-between mb-4 relative z-10">
          <div>
            <p className="label-hud mb-1" style={{color:accentColor}}>Your Profile</p>
            <h1 className="font-display font-extrabold text-2xl leading-none text-[var(--t1)]">
              {user?.arenaName||user?.displayName||'Athlete'}
            </h1>
            <p className="text-xs text-[var(--t3)] mt-1">{disc.emoji} {subDiscipline?.name??disc.name}</p>
          </div>
          <div className="flex items-center gap-2">
            <TierBadge tier={currentTier.id}/>
            <motion.button whileTap={{scale:.88}}
              onClick={()=>navigate('/profile/avatar')} className="btn-icon">
        ...
        <div className="flex gap-1.5 overflow-x-auto scroll-none pb-0.5">
          {TABS.map(({id,label,Icon})=>(
            <motion.button key={id} whileTap={{scale:.9}}
              onClick={()=>setTab(id)}
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-safe scroll-none">
        <AnimatePresence mode="wait">
          <motion.div key={tab}
            ...
            {tab==='overview'  && <OverviewTab accentColor={accentColor} xp={xp} currentTier={currentTier} nextTier={nextTier} tierProgress={tierProgress} avgScore={avgScore}/>}
    ...
    <div className="space-y-3.5 pt-3 pb-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          {v:xp>999?`${(xp/1000).toFixed(1)}k`:xp, l:'XP', c:accentColor},
          ...
          <div key={s.l} className="rounded-2xl p-3 text-center card">
            <p className="font-display font-extrabold text-2xl leading-none tabular" style={{color:(s as any).c||'var(--t1)'}}>{s.v}</p>
      ...
      {nextTier && (
        <div className="card rounded-2xl p-4">
          <div className="flex justify-between mb-2">
            <span className="label-hud">{currentTier.icon} {currentTier.name}</span>
            <span className="label-hud">{nextTier.icon} {nextTier.name}</span>
          </div>
        ...
        <div className="card rounded-2xl p-4">
          ...
          <ResponsiveContainer width="100%" height={90}>
            <LineChart data={chart}>
              ...
              <Tooltip
                contentStyle={{background:'var(--s2)',border:'none',borderRadius:10,fontSize:11,color:'var(--t1)'}}
                formatter={(v:number)=>[v,'Score']}/>
              <Line type="monotone" dataKey="score" stroke={accentColor} strokeWidth={2.5}
                dot={{fill:accentColor,r:3}} activeDot={{r:5}}/>
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
...
const StatsTab = ({accentColor}:{accentColor:string}) => {
  ...
  const weekly = Array.from({length:7},(_,i)=>{
    ...
    return {day:d.toLocaleDateString('en',{weekday:'short'}),count:s.length,avg:s.length?Math.round(s.reduce((a,x)=>a+x.score,0)/s.length):0};
  });
  return (
    <div className="space-y-3.5 pt-3 pb-4">
      <div className="card rounded-2xl p-4">
        ...
        <div className="flex items-end justify-between gap-1.5 h-20">
          {weekly.map(d=>(
            <div key={d.day} className="flex-1 flex flex-col items-center gap-1">
              <motion.div className="w-full rounded-t-md flex-1"
                style={{background:d.count?accentColor:'var(--s3)',minHeight:4}}
                initial={{scaleY:0}} animate={{scaleY:1}} transition={{delay:.05,duration:.5,transformOrigin:'bottom'}}/>
              <p className="label-hud">{d.day}</p>
            </div>
          ))}
        </div>
      </div>
      {sessions.length===0 && <p className="label-hud text-center py-8">Train to see stats</p>}
    </div>
  );
};
      ...
      <div className="card rounded-[20px] p-4">
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl"
            style={{background:`${accentColor}18`}}>{disc.emoji}</div>
          <div>
            <p className="font-display font-bold text-base text-[var(--t1)]">{user?.aiCoachName??'Coach'}</p>
            <p className="label-hud mt-0.5 capitalize">{disc.coachingTone} · {disc.name}</p>
          </div>
        </div>
      </div>
      <div className="card rounded-[20px] overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2.5"
          style={{background:`${accentColor}0d`,borderBottom:'1px solid var(--b1)'}}>
          <p className="label-section">Today's Message</p>
          <motion.button whileTap={{scale:.8,rotate:180}} onClick={refresh}
            className="w-6 h-6 flex items-center justify-center text-[var(--t3)]">↻</motion.button>
        </div>
        <p className="px-4 py-3 text-sm text-[var(--t2)] leading-relaxed italic">
          {tipL?<span className="block h-3 rounded anim-shimmer"/>:tip?`"${tip}"`:'...'}
        </p>
      </div>
      <div className="card rounded-[20px] overflow-hidden">
        <div className="px-4 py-2.5" style={{background:`${accentColor}0d`,borderBottom:'1px solid var(--b1)'}}>
          <p className="label-section">Weekly Plan</p>
        </div>
        <div className="p-4 space-y-2.5">
          {planL ? Array(3).fill(0).map((_,i)=><Skeleton key={i} className="h-12 rounded-xl"/>) :
            plan.length>0 ? plan.map(day=>(
              <div key={day.day} className="rounded-xl p-3" style={{background:'var(--s2)'}}>
                <p className="font-mono text-[10px] font-bold mb-1.5" style={{color:accentColor}}>DAY {day.day} · {day.goal.toUpperCase()}</p>
                {day.drills.map((d,i)=><p key={i} className="text-xs text-[var(--t3)]">· {d}</p>)}
              </div>
  ...
  return (
    <div className="space-y-5 pt-3 pb-4">
      <div className="flex items-center justify-between">
        <p className="label-section">{earned.length}/{ACHIEVEMENTS.length} Unlocked</p>
      </div>
      {cats.map(cat=>{
        ...
        return (
          <div key={cat}>
            <p className="label-hud mb-2 capitalize">{cat} · {unlocked.length}/{list.length}</p>
            <div className="grid grid-cols-3 gap-2">
              {list.map(a=>{
                const on=earned.includes(a.id);
                const c=RARITY_C[a.rarity]||'#6b7280';
                return (
                  <motion.div key={a.id} whileTap={{scale:.92}}
                    className="rounded-2xl p-3 text-center transition-all"
                    style={on
                      ?{background:`${c}10`,border:`1px solid ${c}40`,boxShadow:`0 0 12px ${c}15`}
                      :{background:'var(--s1)',border:'1px solid var(--b0)',opacity:.38}}>
                    <p className="text-2xl">{a.secret&&!on?'❓':a.icon}</p>
                    <p className="font-mono text-[9px] font-bold mt-1 leading-tight" style={{color:on?c:'var(--t4)'}}>
                      {a.secret&&!on?'???':a.name}
                    </p>
    ...
    <div className="space-y-3.5 pt-3 pb-4">
        ...
        {soundEnabled&&(
          <div>
            <div className="flex justify-between mb-2">
              ...
              <p className="font-mono text-xs text-[var(--t3)]">{Math.round(masterVolume*100)}%</p>
            </div>
            <Slider value={[masterVolume]} onValueChange={([v])=>setMasterVolume(v)} min={0} max={1} step={.05}/>
          </div>
        )}
      </div>
      <div className="card rounded-[20px] p-4 space-y-4">
      ...
      <div className="card rounded-[20px] p-4 space-y-2">
        ...
        {[
          ...
          <motion.button key={r.label} whileTap={{scale:.97}} onClick={r.action}
        ...
        <motion.button whileTap={{scale:.97}}
          ...
          className="w-full flex items-center gap-3 p-3 rounded-2xl mt-1"
...
const SettingRow = ({label,icon,children}:{label:string;icon:React.ReactNode;children:React.ReactNode}) => (
  <div className="flex items-center justify-between">
    <div className="flex items-center gap-3">
