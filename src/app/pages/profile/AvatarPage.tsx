...
import { SubDisciplineSelector } from '@shared/components/arena/SubDisciplineSelector';
...
export default function AvatarPage() {
  ...
  const save = async () => {
    setSaving(true);
    updateUser({ arenaName, bio, discipline: disciplineId, subDiscipline: subDisciplineId });
    ...
    <div className="min-h-screen bg-void pb-24">
      ...
      <div className="px-5 space-y-6">
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="w-24 h-24 rounded-3xl flex items-center justify-center text-5xl relative"
            style={{ background: `${selectedDisc.color}20`, border: `2px solid ${selectedDisc.color}40` }}>
            {selectedDisc.emoji}
        ...
        {/* Arena name */}
        <div>
          <label className="text-xs font-mono text-t3 uppercase tracking-widest block mb-2">Arena Name</label>
          <input
            value={arenaName}
            onChange={(e) => setArenaName(e.target.value)}
            maxLength={20}
            placeholder={user?.displayName ?? 'Your arena name…'}
            className="w-full bg-s1 border border-b1 rounded-xl px-4 py-3 text-t1 placeholder:text-t3 focus:outline-none focus:border-opacity-60"
          />
        </div>

        {/* Bio */}
        <div>
          <label className="text-xs font-mono text-t3 uppercase tracking-widest block mb-2">Bio</label>
          <textarea
            value={bio}
            ...
            rows={3}
            className="w-full bg-s1 border border-b1 rounded-xl px-4 py-3 text-t1 placeholder:text-t3 focus:outline-none resize-none"
          />
        </div>

        {/* Discipline picker */}
        <div>
          <label className="text-xs font-mono text-t3 uppercase tracking-widest block mb-2">Discipline</label>
